...
const database$ = U.skipUnless(R.identity, U.toProperty(_database)); // Constants

const events = ['connecting', 'connected', 'open', 'error', 'disconnecting', 'disconnected', 'reconnected']; //

const events$ = U.thru(database$, U.mapValue(db => R.map(ev => K.fromEvents(db, ev).map(payload => ({
  payload,
  ev
})), events))); //
