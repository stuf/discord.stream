...
const L = require('partial.lenses'); // const logger = require('./logger');
...
const handleLog = (command, args, message) => {
  try {
    ...
    const getThumbnails = L.collect([L.entries, L.reread(([size, thumb]) => Object.assign({}, {
      size
    }, thumb))]);
    ...
    return U.thru(K.constant(id), Youtube.methods.getVideoJson, U.mapValue(L.get(['data', M.Youtube.basicVideoL])), U.mapValue(R.compose(response => ({
      response,
      command,
      message
    }), ({
