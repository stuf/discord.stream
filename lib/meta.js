...
const {
  invoke0,
  construct1
} = require('./shared'); // Generic


const Generic = {};
Generic.readSearchParamsL = L.reread(R.compose(R.fromPairs, Array.from, invoke0('entries')));
module.exports.Generic = Generic; // Discord client meta models

/**
 * Function for parsing a string into a list of tokens.
 */

const parseCommand = R.compose(R.split(' '), R.drop(1), R.trim());
...
Youtube.basicVideoL = ['items', L.first, L.pick({
  id: 'id',
  url: ['id', L.normalize(R.concat('https://youtu.be/'))],
  title: ['snippet', 'title'],
  etag: 'etag',
...
Youtube.contentDuration = ['items', L.first, 'contentDetails', 'duration'];
module.exports.Youtube = Youtube; // Database meta models
//
